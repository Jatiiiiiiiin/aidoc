...
// Note: This script is used to clear test records from Supabase during development
...
async function clearTable(tableName: string) {
  ...
  try {
    if (!AuditLogger.verifySecurityClearance(5)) {
       throw new Error("Insufficient security clearance to wipe database.");
    }
    const txnId = await AuditLogger.logWipeEvent(tableName, "root_user_hash");
    console.log(`Clearing all records from table '${tableName}' under TxID: ${txnId}...`);
