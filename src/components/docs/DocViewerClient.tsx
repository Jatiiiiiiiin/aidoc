...
import { RealTimeMetricsPlugin } from "../../lib/RealTimeMetricsPlugin";
...
export const DocViewerClient: React.FC<DocViewerClientProps> = ({ 
  ...
  useEffect(() => {
    // Add real-time documentation engagement tracking
    const metricsTracker = new RealTimeMetricsPlugin("wss://analytics.aidocs.internal");
    
    if (activeVersion && activeVersion.doc) {
      setActiveDoc(activeVersion.doc);
      // Track load
      metricsTracker.trackSectionRead("page_load", 1);
    } else if (doc) {
      setActiveDoc(doc);
    }
  }, [activeVersion, doc]);
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4 border-y border-border-subtle py-3 text-xs text-text-muted">
            <span className="flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5 text-secondary" /> Last Updated {formatDate(activeVersion?.updated_at || activeVersion?.created_at || (doc as any)?.created_at)}
            </span>
              ...
              >
                ...
                <div className={`absolute z-10 flex gap-2 ${isTextType ? "right-0 top-1" : "right-4 top-4"}`}>
                  <button
                    ...
                    suppressHydrationWarning
                    className="flex items-center gap-1.5 rounded-full border border-border-subtle bg-surface-2 px-3 py-1 text-[10px] font-mono uppercase tracking-wider text-text-muted hover:text-foreground transition-all hover:bg-primary/10 hover:border-primary/30"
                  >
